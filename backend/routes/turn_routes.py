...
def api_create_turn():
    ...
    # Default behavior: execute instantly from roster action.
    auto_execute = data.get('auto_execute', True)
    if auto_execute:
        engine = TurnBookingEngine(universe)
        crowd_reaction = CrowdReaction(data.get('crowd_reaction', 'heat'))
        show_name = data.get('show_name', f"Week {universe.current_week} Show")
        show_id = data.get('show_id', f"show_y{universe.current_year}_w{universe.current_week}")
        result = engine.execute_immediate_turn(
            turn=turn,
            show_id=show_id,
            show_name=show_name,
        ...
        return jsonify({'success': True, 'turn': result['turn'], 'auto_executed': True})

    universe.save_turn_state()
    return jsonify({'success': True, 'turn': turn.to_dict(), 'auto_executed': False})
